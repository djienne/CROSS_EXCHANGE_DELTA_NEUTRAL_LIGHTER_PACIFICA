...
class BotConfig:
    ...
    liquidation_check_grace_period_seconds: int = 30
...
async def scan_symbols_for_positions(lighter_api_client: lighter.ApiClient, pacifica_client: PacificaClient,
    ...
    for i, symbol in enumerate(symbols, 1):
        try:
            ...
            logger.error(f"[{i}/{len(symbols)}] Critical error checking {symbol}: {e}")
            # Re-raise exception to prevent partial/false scan results
            raise
...
async def recover_state(state_mgr: StateManager, lighter_api_client: lighter.ApiClient,
    ...
    try:
        ...
        logger.error(f"{Colors.RED}State recovery failed due to API error: {e}. Entering ERROR state.{Colors.RESET}")
    ...
    async def monitor_position(self):
        ...
        try:
            ...
            try:
                ...
                # ==================== Liquidation / Missing Position Check ====================
                # Grace period to allow API to update after opening
                time_since_open = datetime.now(UTC) - (opened_at_dt or datetime.now(UTC))
                grace_period = self.config.liquidation_check_grace_period_seconds
                
                if time_since_open.total_seconds() > grace_period:
                    if lighter_size == 0 or pa_size == 0:
                        missing_exch = "Lighter" if lighter_size == 0 else "Pacifica"
                        present_exch = "Pacifica" if lighter_size == 0 else "Lighter"
                        remaining_size = pa_size if lighter_size == 0 else lighter_size
                        
                        logger.error(f"{Colors.RED}{Colors.BOLD}CRITICAL: Liquidation or missing position detected on {missing_exch}!{Colors.RESET}")
                        logger.warning(f"Position on {missing_exch} is 0, but {present_exch} has {remaining_size:.4f}.")
                        logger.warning("Triggering emergency close of remaining leg...")
                        
                        # Close the remaining leg immediately
                        await self.close_position(is_emergency=True)
                        
                        # Set state to ERROR to prevent immediate restart and allow manual inspection
                        self.state_mgr.set_state(BotState.ERROR)
                        return
                else:
                    logger.debug(f"Skipping liquidation check (grace period: {time_since_open.total_seconds():.1f}s < {grace_period}s)")
                # ==============================================================================
