...
class PacificaClient:
    ...
    def get_funding_fees(self, symbols: list[str]) -> Dict[str, float]:
        ...
        for symbol in symbols:
            funding_fees[symbol] = self.get_funding_rate(symbol)
        return funding_fees
    ...
    def get_available_balance(self) -> float:
        ...
        try:
            ...
            if not data.get("success"):
                raise ValueError("get_available_balance API call was not successful")

            account_data = data.get("data", {})
            return float(account_data.get("available_to_spend", 0))

        except Exception as e:
            print(f"Warning: Failed to get available balance from API: {e}. Using fallback value of 10000.")
            if self.allow_fallback:
                return 10000.0
            raise RuntimeError("Failed to get available balance from API") from e
    ...
    async def get_mark_price(self, symbol: str) -> float:
        """Get current mark price (asynchronous)."""
        try:
            ...
            # Add other fallbacks as needed
                ...
                # Return 0.0 for unknown symbols to ensure failure downstream
                # if a specific fallback is not available.
                return 0.0
    ...
    async def get_position(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log at DEBUG level if no positions, INFO if positions exist
            if len(positions) > 0:
                logger.debug(f"Pacifica: Received {len(positions)} position(s) for {symbol} check")
            else:
                logger.debug(f"Pacifica: No positions found for {symbol}")
            ...
            print(f"Warning: Failed to get position for {symbol} from API: {e}.")
            # CRITICAL: Raise exception instead of using fallback to prevent false state
    ...
    def place_market_order(
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Pacifica API error response: {error_body}")
                raise RuntimeError(f"Failed to place market order: {e.response.status_code} - {error_body}")
            except:
                logger.error(f"Pacifica API error response (raw): {response.text}")
                raise RuntimeError(f"Failed to place market order: {e.response.status_code} - {response.text}")
        except Exception as e:
    ...
    def set_leverage(self, symbol: str, leverage: int) -> bool:
        """
        Set leverage for a symbol on Pacifica.
        ...
        Note: For open positions, you can only INCREASE leverage, not decrease it.
        """
        ...
        try:
            ...
            if result.get("success"):
                logger.info(f"Successfully set leverage for {symbol} to {leverage}x on Pacifica")
                return True
            else:
                error = result.get("error", "Unknown error")
                logger.error(f"Failed to set leverage for {symbol}: {error}")
                return False
